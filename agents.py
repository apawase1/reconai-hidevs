...
discovery_agent = LlmAgent(
    ...
    instruction=(
        ...
        "only the extracted text matters downstream; "
...
reporting_agent = LlmAgent(
    ...
    instruction=(
        ...
        "total_spent, category_breakdown, category_vendors, subscriptions, "
        ...
        "recurring_count/gst_eligible_total already exclude both "
        "duplicate-flagged AND payment_status 'pending' transactions (a bill "
        "that's merely been generated/issued isn't spend yet) — never present "
        "a second 'raw' or 'row volume' total that adds those back in. "
        ...
        "2) Subscriptions you have — list from `subscriptions`, one line "
        "each with vendor and amount; "
        "3) Recurring investments — list from `recurring_investments`, same "
        "format; "
        "4) Payments pending — list from `payments_pending` if non-empty "
        "(bills generated/due but not yet paid — make clear these are NOT "
        "included in total_spent above and still need action); "
        "5) 2-4 concrete recommendations based on what the report flagged "
